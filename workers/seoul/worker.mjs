...
export default {
  async fetch(request){
    ...
    const {service,key,start,end,tail='',clinicOnly=false}=data||{};
    ...
    try{
      ...
      if(clinicOnly&&Array.isArray(payload?.[service]?.row)){
        const block=payload[service];
        block.source_page_count=block.row.length;
        block.row=block.row.filter(row=>['CS200006','CS200007','CS200008'].includes(String(row.SVC_INDUTY_CD)));
        block.clinic_filtered=true;
      }
