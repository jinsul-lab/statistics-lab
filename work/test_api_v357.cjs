...
const source=fs.readFileSync('jinsulmap/JINSUL_MAP_v3.5.7.html','utf8');
...
fs.writeFileSync('work/wangsimni-test-results-v357.json',JSON.stringify(wangsimniResult,null,2));
...
fs.writeFileSync('work/api-test-results-v357.json',JSON.stringify({executedAt:new Date().toISOString(),type:'Offline regression; stubbed API responses, not live service validation',results},null,2));
