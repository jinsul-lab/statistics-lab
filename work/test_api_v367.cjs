...
const source=fs.readFileSync('jinsulmap/JINSUL_MAP_v3.6.7.html','utf8');
const prior=fs.readFileSync('jinsulmap/JINSUL_MAP_v3.6.6.html','utf8');
const results=[];
...
await test('All v3.6.6 static IDs preserved; no duplicates',()=>{assert.ok(ids(prior).length>0);assert.equal(new Set(ids(source)).size,ids(source).length);assert.ok(ids(prior).every(id=>ids(source).includes(id)));});
...
await test('Existing credentials, SDK libraries and R-ONE endpoint preserved',()=>{
  ...
  assert.ok(JSON.stringify(config(source))===JSON.stringify(config(prior)),'API configuration changed');
...
fs.writeFileSync('work/wangsimni-test-results-v367.json',JSON.stringify(wangsimniResult,null,2));
...
fs.writeFileSync('work/api-test-results-v367.json',JSON.stringify({executedAt:new Date().toISOString(),type:'Offline regression; stubbed API responses, not live service validation',results},null,2));
