const fs=require('fs'),vm=require('vm'),assert=require('node:assert/strict');const src=fs.readFileSync('jinsulmap/JINSUL_MAP_v3.5.5.html','utf8');
...
c.URL=URL;c.document={getElementById:()=>null};c.scanApiErrorMessage=e=>e.message||e.name;c.scanEscapeHTML=s=>String(s);
vm.runInContext(fs.readFileSync('work/network_v355.js','utf8'),c);
vm.runInContext(fs.readFileSync('work/charts_v355.js','utf8'),c);
let fi;Object.defineProperty(c,'fetch',{get:()=>async(...args)=>{const r=await fi(...args);if(r&&!r.text&&r.json)r.text=async()=>JSON.stringify(await r.json());return r;},set:v=>fi=v});
(async()=>{
