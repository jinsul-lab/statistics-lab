...
const source=fs.readFileSync('JINSUL_MAP_v3.5.2.html','utf8');
...
const c=context(['scanNumber','scanApiCodeError','scanApiErrorMessage','scanApiEnvelope','scanRoneEnvelope','scanMedian','scanFetchRealty','scanRequestText','scanFetchWithTimeout','scanFetchSeoulCommerceRows'],{SEOUL_API_BASES:['https://example.test'],SEOUL_COMMERCE_API_KEY:'fixture',location:{protocol:'https:'}});
...
await test('Seoul unknown schema rejected',async()=>{c.fetch=async()=>({ok:true,text:async()=>'{}'});await assert.rejects(c.scanFetchSeoulCommerceRows('fixture',1,5),/구조/);});
await test('Seoul explicit no-data response accepted',async()=>{c.fetch=async()=>({ok:true,text:async()=>'{"RESULT":{"CODE":"INFO-200"}}'});assert.equal((await c.scanFetchSeoulCommerceRows('fixture',1,5)).total,0);});
...
const geo=context(['scanHaversineMeters','scanIsSeoulCandidate','scanNearestSeoulHotspots','scanWgs84ToWtm','scanNearestTradeAreas']);
...
fs.writeFileSync('work/wangsimni-test-results.json',JSON.stringify(wangsimniResult,null,2));
...
fs.writeFileSync('work/api-test-results.json',JSON.stringify({executedAt:new Date().toISOString(),type:'Offline regression; stubbed API responses, not live service validation',results},null,2));
