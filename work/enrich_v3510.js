...
async function scanFetchEgen(place){
  ...
  let payload;try{payload=await scanFetchApiDocument('https://apis.data.go.kr/B552657/HsptlAsembySearchService/getHsptlMdcncListInfoInqire?'+params,30000);}catch(error){if(error.httpStatus===403||/HTTP.*\b403\b/i.test(error.message||'')){scanEgenApprovalError=true;throw Error('국립중앙의료원 API HTTP 403 · 활용승인 및 인증키 확인 필요');}throw error;}
