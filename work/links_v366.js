...
scanSitePreview=function(){
  ...
  sitePhotoPanos.get(key).then(id=>{
    ...
    slot.textContent='';const viewer=new kakao.maps.Roadview(slot);const target=new kakao.maps.LatLng(c.lat,c.lng);kakao.maps.event.addListener(viewer,'init',()=>{const view=viewer.getProjection().viewpointFromCoords(target,0);viewer.setViewpoint(view);});viewer.setPanoId(id,target);
