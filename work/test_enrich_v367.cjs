...
vm.runInContext(fs.readFileSync('jinsulmap/JINSUL_MAP_v3.6.7.html','utf8').match(/^function scanSiteAutoFill[^]*?^}/m)[0],c);
...
test('complete zero building count auto retained',()=>{const d={fields:{}};c.scanSiteAutoFill(d,{candidate:{},building:{parking:0,elevators:0}});assert.ok(d.fields.parking.includes('0대'))});
...
(async()=>{
  ...
  fs.writeFileSync('work/enrich-test-results-v367.json',JSON.stringify({type:'Local and mocked tests, not live API',passed:n},null,2));console.log(n+' passed');
